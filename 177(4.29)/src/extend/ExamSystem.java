...
public class ExamSystem {
	...
	public void inputPaper(){
		...
		//2.录入多个多选题
		MutilChoice[] mutils = getMutilChoices();
		//3.录入试卷
	...
	public SingleChoice[] getSingleChoices(){
		 ...
		 //创建保存单选题的数组
		 SingleChoice[] singles = new SingleChoice[num];
		 
		 for(int i =0 ; i< num ; i++){
			 //2.录入单选题
			 SingleChoice single = getSingleChoice();
			 //把单选题保存到数组中
			 singles[i] = single;
		 }
	...
	public SingleChoice getSingleChoice(){
		...
		//创建单选题对象
		SingleChoice single = new SingleChoice(num,question,null,correct,A,B,C,D,score);
		return single;
	}
	
	//获取多个多选题
	public MutilChoice[] getMutilChoices(){
		return null;
	}
	...
	public void exam(){
		...
		//1.将试卷中的单选题输出
		SingleChoice[] singles = this.p.getSingles();
		//遍历数组将数组中的元素输出
		for(int i =0 ;i < singles.length;i ++){
			SingleChoice single = singles[i];
			//输出单选题
